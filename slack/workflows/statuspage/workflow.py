...
class Statuspage(WorkflowClass):
    configuration_values = ['ORGANIZATION_ID', 'STATUSPAGEIO_API_KEY', 'STATUSPAGEIO_PAGE_ID']

    def enable(self, workflow: Workflow):
        params = dict((x.name, x.value) for x in workflow.parameters.all())

        required = ['ORGANIZATION_ID', 'STATUSPAGEIO_API_KEY', 'STATUSPAGEIO_PAGE_ID']
        if not all([ params[x] for x in params.keys() & required]):
            logger.warn("Not all required parameters are set for statuspage workflow.")
            return

        set_status_page_conn(statuspageio.Client(organization_id=params['ORGANIZATION_ID'], api_key=params['STATUSPAGEIO_API_KEY'],

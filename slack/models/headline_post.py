...
class HeadlinePost(models.Model):
    ...
    def update_in_slack(self):
        ...
        msg.add_block(Section(block_id="second_row",fields=[Text(f":male-firefighter:  Incident Lead: {incident_lead_text}"), Text(f"{self.incident.status_emoji()} Status: {self.incident.status_text().capitalize()}")]))
        # Incident reporter and severity
        severity_text = self.incident.severity_text().capitalize() if self.incident.severity_text() else "-"
        msg.add_block(Section(block_id="third_row", fields=[Text(f":man-raising-hand: Reporter: {user_reference(self.incident.reporter.external_id)}"), Text(f"{self.incident.severity_emoji()} Severity: {severity_text}")]))
        ...
        if channel_ref:
            msg.add_block(Section(block_id="fourth_row", fields=[Text(f":speech_balloon: Comms Channel: {channel_ref or '-'}"), Text(f":newspaper: Document: <{doc_url}|Incident {self.incident.pk}>")]))
